...
type problem0054 struct {
    id int
    description string
}

func (p problem0054) ID() int {
    return p.id
}

func (p problem0054) Description() string {
    return p.description
}

func (p problem0054) Solve() string {
    hands := []string{
    ...
    for _, hand := range hands {
        player1 := newHandFromString(hand[:14])
        player2 := newHandFromString(hand[15:])

        cmp := player1.CompareHands(player2)
    ...
    return strconv.Itoa(player1Wins)
}
...
var suitMapping map[string]int = map[string]int{"C": clubs, "D": diamonds, "H": hearts, "S": spades}
var rankMapping map[string]int = map[string]int{"A": ace, "2": two, "3": three, "4": four, "5": five, "6": six, "7": seven, "8": eight, "9": nine, "T": ten, "J": jack, "Q": queen, "K": king}
...
func (c *card) SetString(s string) {
    rank, suit := s[:1], s[1:]

    c.rank = rankMapping[rank]
    c.suit = suitMapping[suit]
}
...
func (h *hand) SetString(s string) {
    h.cards = make([]card, 0, 5)

    cardStrings := strings.Split(s, " ")

    for _, cardString := range cardStrings {
        c := newCardFromString(cardString)
...
func (h hand) Score() score {
    handScore := score{highCard, getCardRankingOrder(h)}

    isRoyalFlush := isHandRoyalFlush(h)
    isFourOfAKind := isHandFourOfAKind(h)
    isFullHouse := isHandFullHouse(h)
    isFlush := isHandFlush(h)
    isStraight := isHandStraight(h)
    isThreeOfAKind := isHandThreeOfAKind(h)
    isTwoPairs := isHandTwoPairs(h)
    isOnePair := isHandOnePair(h)

    if isRoyalFlush {
        handScore.rank = royalFlush
    } else if isFourOfAKind {
        handScore.rank = fourOfAKind
        handScore.cardRankOrder = getFourOfAKindOrder(h)
    } else if isFullHouse {
        handScore.rank = fullHouse
        handScore.cardRankOrder = getFullHouseOrder(h)
    } else if isFlush && isStraight {
        handScore.rank = straightFlush
    } else if isFlush {
        handScore.rank = flush
    } else if isStraight {
        handScore.rank = straight
    } else if isThreeOfAKind {
        handScore.rank = threeOfAKind
        handScore.cardRankOrder = getThreeOfAKindOrder(h)
    } else if isTwoPairs {
        handScore.rank = twoPairs
        handScore.cardRankOrder = getTwoPairsOrder(h)
    } else if isOnePair {
        handScore.rank = onePair
        handScore.cardRankOrder = getOnePairOrder(h)
    }

    return handScore
}

func getCardRankingOrder(h hand) []int {
    order := make([]int, 0, 5)

    sort.Slice(h.cards, func(i int, j int) bool { return h.cards[i].rank > h.cards[j].rank })

    for _, card := range h.cards {
        order = append(order, card.rank)
    }

    return order
}

func isHandOnePair(h hand) bool {
...
func getOnePairOrder(h hand) []int {
    order := make([]int, 5)
    ...
    for rank, count := range ranks {
        if count == 2 {
            ...
            if rank > order[2] {
                order[2], order[3], order[4] = rank, order[2], order[3]
            } else if rank > order[3] {
                order[3], order[4] = rank, order[3]
            } else {
                order[4] = rank
            }
        }
...
func isHandTwoPairs(h hand) bool {
    ...
    for _, count := range ranks {
        if count == 2 && !twoTheSame1 {
            twoTheSame1 = true
        } else if count == 2 && twoTheSame1 {
            twoTheSame2 = true
            break
        }
    }

    return twoTheSame1 && twoTheSame2
}

func getTwoPairsOrder(h hand) []int {
    order := make([]int, 5)
...
func isHandThreeOfAKind(h hand) bool {
...
func getThreeOfAKindOrder(h hand) []int {
    order := make([]int, 5)
...
func isHandStraight(h hand) bool {
    consecutiveOrder := true
...
func isHandFullHouse(h hand) bool {
    ...
    return threeTheSame && twoTheSame
}

func getFullHouseOrder(h hand) []int {
    order := make([]int, 5)
...
func isHandFourOfAKind(h hand) bool {
...
func getFourOfAKindOrder(h hand) []int {
    order := make([]int, 5)
...
func isHandRoyalFlush(h hand) bool {
    royalConsecutiveOrder := isHandStraight(h) && isHandFlush(h)

    if royalConsecutiveOrder {
        sort.Slice(h.cards, func(i int, j int) bool { return h.cards[i].rank < h.cards[j].rank })

        royalConsecutiveOrder = h.cards[0].rank == ten
    }

    return royalConsecutiveOrder
}

func (h hand) CompareHands(o hand) int {
    cmp := 0
    ...
    if firstScore.rank > secondScore.rank {
        cmp = 1
    } else if firstScore.rank < secondScore.rank {
        cmp = -1
    } else {
        for i := 0; i < len(firstScore.cardRankOrder) && i < len(secondScore.cardRankOrder); i++ {
            if firstScore.cardRankOrder[i] > secondScore.cardRankOrder[i] {
                cmp = 1
            } else if firstScore.cardRankOrder[i] < secondScore.cardRankOrder[i] {
                cmp = -1
            }
...
func newHandFromString (s string) hand {
    var h hand
    h.SetString(s)
...
func init() {
    Registry[54] = problem0054{54, "How many poker hands does Player 1 win?"}
}
